"""Train a reward classifier on ordered frame sequences.

Unlike the single-window reward classifier, this model sees a short ordered
sequence near the end of each episode. It is intended to classify stable final
success, not whether the object was ever grasped once.
"""
...
DEFAULT_OUTPUT_DIR = Path("outputs/reward_sequence_classifier/red_cube_v0")
...
class RewardSequenceDataset(Dataset):
    def __init__(
        self,
        roots: list[Path],
        image_key: str,
        ...
        self.sequence_span_frames = sequence_span_frames
        self.window_end_offset_frames = window_end_offset_frames
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ref_index = self.episode_indices[index]
        ref = self.episode_refs[ref_index]
        frame_indices = sequence_frame_indices(
            ref.start,
            ref.end,
            self.sequence_length,
            self.sequence_span_frames,
            self.window_end_offset_frames,
            self.sequence_scope,
        )
        dataset = self.datasets[ref.source_index]
        images = []
        states = []
        for frame_index in frame_indices:
            item = dataset[frame_index]
            images.append(item[self.image_key])
        ...
        return {
            ...
            "episode_ref_index": torch.tensor(ref_index, dtype=torch.long),
        }


class TinyRewardSequenceClassifier(nn.Module):
    def __init__(self, state_dim: int, image_size: tuple[int, int], hidden_dim: int = 128) -> None:
        super().__init__()
        ...
        self.frame_proj = nn.Sequential(
            nn.Linear(96 * 4 * 4 + state_dim, hidden_dim),
            nn.ReLU(inplace=True),
        )
        self.gru = nn.GRU(
    ...
    def forward(self, image: torch.Tensor, state: torch.Tensor) -> torch.Tensor:
        batch, seq, channels, _, _ = image.shape
        image = image.flatten(0, 1)
        image = F.interpolate(image, size=self.image_size, mode="bilinear", align_corners=False)
        visual = self.cnn(image).view(batch, seq, -1)
        frame_features = self.frame_proj(torch.cat([visual, state], dim=-1))
        output, _ = self.gru(frame_features)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--sequence-span-frames",
        type=int,
        default=120,
        help="For tail scope, how many frames before the chosen window end the sequence spans.",
    )
    parser.add_argument(
        "--sequence-scope",
        choices=("tail", "full"),
        default="tail",
        help="Sample from the tail window or uniformly across the full episode.",
    )
    parser.add_argument(
        "--window-end-offset-frames",
        type=int,
        default=0,
        help="Skip this many final frames before ending the sampled sequence.",
    )
...
def sequence_frame_indices(
    start: int,
    end: int,
    sequence_length: int,
    sequence_span_frames: int,
    window_end_offset_frames: int,
    sequence_scope: str = "tail",
) -> list[int]:
    window_end = max(start + 1, end - max(window_end_offset_frames, 0))
    if sequence_scope == "full":
        window_start = start
    else:
        window_start = max(start, window_end - max(sequence_span_frames, sequence_length))
    if window_end <= window_start:
        return [start] * sequence_length
    raw = np.linspace(window_start, window_end - 1, num=sequence_length)
...
def load_state_normalizer(
    ...
    for ref_index in train_indices:
        ...
        indices = sequence_frame_indices(
            ref.start,
            ref.end,
            sequence_length,
        ...
        states.append(np.stack(data.iloc[indices]["observation.state"].to_numpy()).astype("float32"))
...
def save_config(args: argparse.Namespace, manifest: dict, state_norm: Normalizer, train_eps: int, val_eps: int) -> None:
    ...
    config = {
        ...
        "sequence_span_frames": args.sequence_span_frames,
...
def evaluate(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            correct_count += int((pred == label).sum().item())
    ...
    metrics = {
        ...
        "episode_accuracy": correct_count / max(total_count, 1),
    }
...
def main() -> int:
    ...
    val_dataset = RewardSequenceDataset(
        roots,
        image_key,
        episode_refs,
        val_ep_indices,
    ...
    train_labels = torch.tensor([episode_refs[idx].label for idx in train_ep_indices], dtype=torch.float32)
    pos = train_labels.sum().clamp_min(1.0)
    neg = (train_labels.numel() - train_labels.sum()).clamp_min(1.0)
    ...
    print(
        ...
        f"sequence_span_frames={args.sequence_span_frames}, "
        f"sequence_scope={args.sequence_scope}, "
    ...
    with metrics_path.open("w", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["epoch", "train_loss", "val_loss", "val_episode_acc"],
        )
        writer.writeheader()
